...
import './App.css';
import { ConnectToRoom, SendMessage, GenerateRoomID, Disconnect, GetMyPublicKeyFingerprint, SetPeerFingerprint, GetPeerFingerprint, GetPeerKeyFingerprint } from "../wailsjs/go/main/App";
...
function App() {
    ...
    const [peerFingerprints, setPeerFingerprints] = useState<Map<string, string>>(new Map());
    ...
    useEffect(() => {
        const messageCallback = (userId: string, username: string, content: string) => {
        ...
        const peerJoinCallback = async (userId: string, username: string, fingerprint?: string) => {
            if (fingerprint) {
                ...
                if (savedFingerprint && savedFingerprint !== fingerprint) {
                    console.warn(`Key fingerprint mismatch for ${username} (${userId}): expected ${savedFingerprint}, got ${fingerprint}`);
        ...
        const keyMismatchCallback = (userId: string, username: string, expectedFingerprint: string, receivedFingerprint: string) => {
            console.warn(`SECURITY WARNING: Key fingerprint mismatch for ${username} (${userId})`);
            console.warn(`Expected: ${expectedFingerprint}`);
            console.warn(`Received: ${receivedFingerprint}`);
            alert(`${t('security.keyMismatch')} ${username}\n${t('security.expected')}: ${expectedFingerprint}\n${t('security.received')}: ${receivedFingerprint}`);
        };
    ...
    const createNewChat = async () => {
        ...
        try {
            await ConnectToRoom(nodeUrl, newRoomID, username);
            setConnected(true);
        } catch (error) {
    ...
    const joinChat = async () => {
        ...
        try {
            await ConnectToRoom(nodeUrl, roomID, username);
            setConnected(true);
            ...
            <div className="chat-container">
                <div className="chat-header">
                    <div className="room-info">
                        ...
                        <span className="peers-count">{peers.length + 1} {t('chat.online')}</span>
